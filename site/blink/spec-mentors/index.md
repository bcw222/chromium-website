...
process of writing a specification that will eventually land in Chromium, or to
improve their knowledge if they've already done it a couple times. You shouldn't
write their specification for them. You don't need to help them navigate the
parts of the launch process that don't intersect with the standards process. You
should try to be aware of their schedule and point out things they're doing too
late.

If you ever don't know the answer to a question, mail the other spec mentors at
[spec-mentors@chromium.org](spec-mentors@chromium.org).
...
problems the feature will solve for end users. The initial explainer should
focus on explaining the problem and *not* on describing a particular solution to
the problem.

#### Picking an incubation venue

Think about which standards body the feature is eventually likely to migrate to.
You don't have to be certain at this stage; things can move around. Each
standards body has their own way of doing incubation:
...
toward [Intent to
Ship](http://localhost:8080/blink/launching-features/#new-feature-prepare-to-ship),
