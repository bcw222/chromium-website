...
email](/blink/launching-features/#new-feature-prepare-to-ship). The API owners
may ask you whether you think the specification is ready to ship. If you're
unsure about any of this review, email for help on the
[spec-mentors@chromium.org](mailto:spec-mentors@chromium.org) list, and another
mentor will be happy to help you. Look for the following things, in addition to
using your general good judgement and experience with writing specifications:
...
* The behavior the features should be specified using algorithms that are
  approximately as rigorous as existing features of the web platform. External
  references should follow the [W3C's normative references
...
* If the specification proposes a change to another web standard, it should
  ...
  merge into the upstream specification.
...
* Some web standards are especially hard to integrate with. If the specification
  you're reviewing calls into [Fetch](https://fetch.spec.whatwg.org/), check
  that it follows the guidance in the [Fetch section on using it from other
