...
signals](https://bit.ly/blink-signals) because they can give us the most direct
indications of [interoperability risk](#interoperability-risk) and
[specification](#specification) quality. The feedback we get from the other
...
* *Spec quality*: When other implementers point out parts of the
  [specification](#specification) that are ambiguous or under-specified, the
  ...
  serious issues will sometimes come up after the spec's quality is improved.
...
* *Ergonomic concerns*: Any reviewer, including other implementers, can notice
  that an API won't work well for developers. Our [origin
  trial](/blink/origin-trials) process is supposed to catch this class of
  problem before we get to the I2S stage, but it doesn't always. We have to
  ...
  with live traffic.
* *Values disagreements*: Sometimes other implementers want to make different
  tradeoffs about which Web features are worth adding. In these cases, we need
  to work to fully explore the disagreement and ensure there's no compromise
  that could enable all browsers to satisfy the use case. However, if no
  compromise is available, and the feature significantly advances the Web even
  without interoperability, the API owners will often approve it despite the
  other browser engine's opposition.
