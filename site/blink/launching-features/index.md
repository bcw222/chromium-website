...
Each request to extend beyond that limit may only be for *3 milestones* at a time, and
will not be approved unless substantial progress is demonstrated in all of these areas:
* Draft spec (early draft is ok, but must be spec-like and associated with the appropriate
...
Each subsequent request to extend an origin trial must provide substantial *additional* 
progress on top of the previous extension request.

[Note: the required breaking period was removed in April 2022. This removal will run for 12 months,
after which the API owners will consider making the change permanent.]
