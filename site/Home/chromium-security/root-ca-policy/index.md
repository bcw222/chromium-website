...
\* while existing CA certificates trusted by Chrome *may* have EKU values as described in this table, the Chrome Root Store will only accept applicant root CA certificates that are part of PKI hierarchies [dedicated to TLS certificate issuance](#4-dedicated-tls-pki-hierarchies)
