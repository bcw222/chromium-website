...
These instructions describe how a web developer can perform end-to-end testing of their sites in Chromium, while forcing Chromium to treat those sites as members of a Related Website Set, without needing to publicly establish a Related Website Set in Chromium's distribution list.
...
1. Enable the Related Website Set flag. For the flag value, enter a comma-separated list of domains. (Note that all domains must be HTTPS.) E.g.: `https://rws-owner.example,https://rws-member1.example,https://rws-member2.example`. This flag can also be enabled by appending e.g. `--use-first-party-set="https://rws-owner.example,https://rws-member1.example,https://rws-member2.example"` to Chromium's command-line.
...
Related Website Sets will begin an origin trial in M89.

The goals of the origin trial are to:

* Test RWS functionality within a limited prototype, including the `SameParty` attribute
* Build awareness/interest in the RWS feature.
* Receive set membership requests and determine if RWS policy needs to be adjusted in order to meet privacy norms.
* Determine if RWS functionality meets needs of common-owned, separate-domain entities (during or post-OT)
* Test RWS UI options for user visibility and usefulness

## Origin Trial Policy

In order to apply a structured approach to examining user understanding of RWS relationships, the following policy constraints will apply to the Related Website Sets origin trial:

* Related Website Sets during OT will be limited to five registrable domains (plus any TLD variants of those five). This allows for viable testing of RWS functionality, and a small set size to initially gauge user understanding of the feature.
* An individual domain may only be included in a single Set.
* Related Website Sets during OT will only be available to common-owned, common-controlled domains. Common ownership and control has been proposed to aid in user understanding of RWS relationships.
* Related Website Set requests during OT will specify the location of the privacy policy or privacy policies of their proposed set members, and will list any differences in those policies.

## Origin Trial Functionality

The RWS origin trial will be “cosmetic” in that it will not change data sharing capabilities for RWS member domains; therefore, UX treatments will not be required for initial OT. In addition, when the user has third-party cookie blocking enabled, Chrome's normal functionality will persist and cookies will not be shared in cross-domain contexts - even if the domains are part of the Related Website Set OT.

In parallel with the origin trial, we will be conducting user research to better understand user expectations with respect to Related Website Sets. It is expected that this will allow testing of browser user interface options to make Related Website Sets discoverable and transparent for users.

## Joining the Origin Trial

If you are interested in participating in the [origin trial](https://github.com/GoogleChrome/OriginTrials/blob/gh-pages/developer-guide.md) for Related Website Sets and `SameParty`, please follow the below instructions:

1. Identify the members and owner of your organization's Related Website Set.
...
1. Modify each of your sites to serve &lt;site&gt;/.well-known/first-party-set. These files will be used at registration-time to verify opt-in on each site.
    * &lt;owner site&gt;/.well-known/first-party-set should be a JSON file whose content is an object listing the owner and the members. E.g., `{owner: "https://rws-owner.example", members: ["https://rws-member1.example", "https://rws-member2.example"]}`.
    * &lt;member site&gt;/.well-known/first-party-set should be a JSON file whose content is an object listing the owner. E.g., `{owner: "https://rws-owner.example"}`.
    * This is similar to the .well-known machinery described in the Related Website Sets explainer, but does not include assertions or versioning.
1. Follow the standard origin trial signup process for the experiment here: https://developers.chrome.com/origintrials/#/trials/active.
    ...
    * After signup, you will see origin trial tokens on the registration page, but they do not need to be deployed to your site. The tokens will have no effect on the enabling of the RWS functionality.
...
* September 25, 2023: renamed First-Party Sets -> Related Website Sets.
