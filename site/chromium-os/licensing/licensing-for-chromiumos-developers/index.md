...
(cr) ~/chromiumos/chromite/licensing $ ./licenses --debug --board reven -p $pkg_name --generate -o  out-single.html 2>&1 | tee out-single
```

You can generate the HTML license file with:

```none
(cr) ~/chromiumos/chromite/licensing $ ./licenses --debug --board reven --generate-licenses --output  outall.html 2>&1 | tee outall
```
