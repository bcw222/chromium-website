...
SDK: so toolchains binpkgs be uprevved if and only if a new SDK is uprevved.

If the overall SDK generation fails, or if the newly built SDK cannot build each
target ChromeOS architecture, then the SDK is not refreshed and the released
files stay stable for developers.
...
1.  The commit queue launches the "SDK Verifiers":
    `chromeos/cq/chromiumos-sdk-uprev-*-cq`, where * is each target architecture
    that the SDK needs to be able to build. These verifiers will use the new SDK
    and binpkgs to build images from source on each of our target architectures.
    If these tests pass, then the uprev CLs will be merged.
...
*   Downloads the bootstrap version of the SDK and sets up chroot environment.
...
*   Packages the freshly built SDK into a tarball, and uploads it to Google
    Storage.
*   Uploads binpkgs for the amd64-host and the toolchains to Google Storage.
...
[build_sdk.py]: https://chromium.googlesource.com/chromiumos/infra/recipes/+/refs/heads/main/recipes/build_sdk.py "Link to the SDK builder's source code."
[build_sdk.star]: https://chrome-internal.googlesource.com/chromeos/infra/config/+/refs/heads/main/misc_builders/build_sdk.star "Link to the SDK builder's configuration."
...
There is a
[Gitiles poller](https://chromium.googlesource.com/infra/luci/luci-go/+/refs/heads/main/lucicfg/doc/README.md#luci.gitiles-poller)
...
```
cros try -g <cl_1>,<cl_2> chromiumos-sdk-tryjob
```
