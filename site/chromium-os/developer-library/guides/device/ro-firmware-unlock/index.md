...
title: Read-only firmware unlock on 2024+ devices
---
...
Chromebooks launched in or after 2024 were built with a security feature that
...
GalliumOS). You have always needed to disable the firmware write-protection
(e.g. by disconnecting the battery) for this, but on these newer devices you now
also need to disable this read-only firmware verification feature, and this
guide explains how to do that.
...
generally all device families that were first sold in or after 2024. Do
...
your device is connected to an AC charger while doing so.
...
1. If you wish, you can also run `gsctool -a -w disable` at this point to
permanently disable the firmware write-protection. This means you no longer need
to disconnect the battery to overwrite read-only firmware.
...
longer boot. You can use the following key combination to recover from this
state:

1. Press and hold the power button

1. Press the Refresh (F2) button twice
...
> **NOTE:** This key combination will only disable read-only firmware
