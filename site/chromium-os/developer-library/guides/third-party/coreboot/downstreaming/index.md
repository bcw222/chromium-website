...
### Copybot
[Copybot] is a custom tool made by the ChromeOS Zephyr team for handling the task of
...
1. Manual
    ...
    1. Send the CLs to the Commit Queue. From the Gerrit CLI:
        ...
        ```
        1. Note: LUCI and the CQ can trip-up when there's ~80+ CLs. If
...
1. Script
    ...
    ```
    2. To clear all the downstreaming CLs from your attention list:
    ```bash
    ...
    ```
    3. When CQ fails midstack
        Downstream up to that commit using:
        ```bash
...
the conflicted change(resolved or not) and apply the skip configuration
