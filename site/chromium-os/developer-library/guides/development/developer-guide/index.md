...
or `-g` flag. Note that if your options contain `-` characters, you must specify
them like so: `-g=--option-with-dash` so that they are not interpreted as
arguments to `cros debug` itself. If your option contains spaces, then it must
be in quotes. See these examples:
