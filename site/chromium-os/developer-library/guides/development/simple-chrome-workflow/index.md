...
`"checkout_src_internal": True` and setup [reclient](go/chrome-linux-build#setup-remote-execution) to speed up builds.
...
For example, a `cros_vm` invocation in the shell-less flow looks like:

```
(shell) ./third_party/chromite/bin/cros_vm --start
```

and a `deploy_chrome` invocation in the shell-less flow looks like:
