...
files from Chrome crashes in `/home/chronos/crash` and places files from other
crashes in `/var/spool/crash/`.

#### Developer builds:

For Chrome processes, `crash_reporter` puts its files (\*.dmp, \*.meta, and
\*.core) in `/home/chronos/crash/`. For other processes,
