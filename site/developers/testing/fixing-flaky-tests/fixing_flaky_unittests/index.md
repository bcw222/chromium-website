...
Chrome has dozens of unit tests suites. They generally have the name
<component>_unittests, e.g., 'views_unittests', and the catch all 'unit_tests'.
Unit tests run in batches of 10 tests per-process. If a test fails, it, and the
remaining tests in the batch of 10 are re-run singly, each in their own process.
If the retry succeeds, the original
failure is called a hidden flake. This type of flake tends to be fairly
reproducible if you run the same set of tests in a single process as the failing
trybot did. To determine what that set of tests is, click on the test results
tab of the failing build page,  click on the failing test, and then open the
task page. Search for the failing test in the raw output. Up to 9 of the
previous tests listed in the output run in the same process as the failing test
(there’s currently no indication of which tests are in the same process in the
log). To run the tests locally, use --test-launcher-filter-file=<file with names
...
The various unit tests suites run batches of 10 tests sequentially in the same
single process. If one test leaves some global state in an unexpected,
non-default state, subsequent tests in the same batch can fail. If a test fails,
it is retried in its own separate process. 
Working backwards from the failure to the nearest global variable used tends to
uncover the root issue. Comparing a failed run to a successful run of the test
in question can also point to the root cause.
...
default state (see “How to add OnTestEnd checks”). Running all the tests in the
...
[267922](https://crrev.com/c/267922), [2918613](https://crrev.com/c/2918613),
...
correctly, resetting the state in the executable’s OnTesStart method is
...
### Data races
If the test is only flaking on tsan trybots, most likely tsan is detecting a
data race in the test.  The most common cause of this error used to be 
initializing a ScopedFeatureList after the test has started up a thread that
checks if a feature is enabled, or destroying a ScopedFeatureList while a
thread that checks if a feature is enabled is still running. 
[3400019](https://crrev.com/c/3400019) fixed the general case of this but the
corner cases have occurred since then. Tests can avoid the issue completely by
initializing the ScopedFeatureList in the test’s constructor, which means
...
declared, in order to enforce creation and destruction order, as in this
example.

Sometimes destruction of an object can race with an other thread trying to
