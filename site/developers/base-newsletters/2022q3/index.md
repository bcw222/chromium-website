...
*   Migration can still be difficult to incrementally land. `base::Value::Dict`
    ...
    *   TBD: Implement `base::Value::Dict::View` and `base::Value::List::View`
        as transient types to ease migration. e.g. a `base::Value::Dict::View`
        would allow passing in `base::DictionaryValue`, `base::Value` (that are
        really dictionaries), or `base::Value::Dict`.
...
*   `raw_ref<T>` is a work in progress, so please keep an eye out for future
    updates.
...
[Example](https://crrev.com/1035144)
