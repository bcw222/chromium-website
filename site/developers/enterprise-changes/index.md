...
#### Can Enterprise set feature directly?
...
a utility function that check both. If the policy value conflicts with the
feature flag setting, the policy value should always take precedence.

```
void IsMyFeatureEnabled(Profile* profile) {
  profile->GetPrefs()->GetBoolean(prefs::kMyFeature)
