...
To prevent the described data loss scenario, it is necessary to keep a copy of
a server-provided proto, including **unknown** fields (in older browser version)
and **partially-supported** fields (e.g. functionality guarded behind a feature
toggle). The logic for caching these protos was implemented in
...
during commits to the server (more details in the [`Implementation`][ImplSection]).

[ImplSection]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#implementation
...
1. Override [`TrimAllSupportedFieldsFromRemoteSpecifics`][TRSFC] function (see this
   [`section`][TrimmingSection]).
2. [Optional] Add DCHECK to local updates flow (see this [`section`][OptionalDcheck]).
3. Include unsupported fields in local changes (see this
   [`section`][LocalChangesSection]).
4. Redownload the data on browser updates (see this [`section`][BrowserUpdateSection]).
5. [Optional] Add sync integration test (see this [`section`][SyncTest]).
...
[BrowserUpdateSection]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#browser-update-flow
[LocalChangesSection]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#local-update-flow
[OptionalDcheck]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#safety-check
[SyncTest]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#integration-test
[TrimmingSection]: https://www.chromium.org/developers/design-documents/sync/old-sync-clients-data-override-protection/#trimming
...
```cpp
sync_pb::EntitySpecifics DataSpecificBridge::TrimRemoteSpecificsForCaching(
...
```cpp
DCHECK_EQ(TrimRemoteSpeficisForCaching(datatype_specifics.ByteSizeLong(), 0u));
```
...
horizon is pretty long (multiple years).
