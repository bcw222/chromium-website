...
a browser update, add the following code to your data-specific Sync bridge:
1. On startup, check whether the unsupported fields cache contains any field
   ...
   the trimming function on cached protos and checking if trims any fields.
...
horizon is pretty long (currently >= M48 clients are being supported).
