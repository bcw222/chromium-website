...
The bridge owns a [`DataTypeChangeProcessor`][MTCP] object, which it uses to
...
to Sync (via the passed-in `DataTypeChangeProcessor`'s `Put` and `Delete`
methods).
...
[ModelReadyToSync]: https://cs.chromium.org/search/?q=ModelReadyToSync+file:/data_type_change_processor.h
...
*   Any local entities that don’t exist remotely must be provided to sync via
    [`DataTypeChangeProcessor::Put`][Put].
...
[Put]: https://cs.chromium.org/search/?q=Put+file:/data_type_change_processor.h
...
The [`DataTypeChangeProcessor`][MTCP] must be informed of any local changes via
...
[ReportError]: https://cs.chromium.org/search/?q=ReportError+file:/data_type_change_processor.h
