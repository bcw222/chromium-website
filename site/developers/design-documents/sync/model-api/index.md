...
`browser_tests`, but are in a separate binary (`sync_integration_tests`)
and run against a fake sync server built into Chrome.
...
cases. There are [plenty of examples][integration_test_examples] for both in the code base.
