...
communicate with synchronously, for example, using `window.open` in
JavaScript. In this case, when Chromium creates a new window or tab, it needs
to reuse the process that the window was opened with. Chromium also has
strategies to assign new tabs to existing processes if the total number of
processes is too large. These strategies are described in [Process
