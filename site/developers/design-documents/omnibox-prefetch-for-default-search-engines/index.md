...
Considering saving resouces, Prerender would attempt to reuse the prefetched
response instead of sending a new network request, which means the search engine
will receive only one main resource request if it recommond to prefetch and
prerender the same term.
