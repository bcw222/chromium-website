...
`ProfileKeyedServiceFactory` derives from `BrowserContextKeyedBaseFactory`
...
noted in the `BrowserContextKeyedBaseFactory` below.
...
*   PKSF provides control over the creation of services for different profile
    ...
    *   PKSF allows you to control which Profile type the service will be
            ...
            default behavior.
    *   More information on Profile Selection can be found [here](https://source.chromium.org/chromium/chromium/src/+/main:chrome/browser/profiles/profile_keyed_service_factory.md;l=72?q=profile_keyed&ss=chromium)
...
Examples of PKSF factory initialisation (constructor definition):
...
// add example of creation of a BCKSF
...
`ProfileKeyedBaseFactory` that defines the most general dependency stuff
while `ProfileKeyedBaseFactory` is a specialization that deals with
normal objects. There is a second `RefcountedProfileKeyedServiceFactory`
that gives slightly different semantics and storage for `RefCountedThreadSafe`
objects. Equivalent `RefcountedBrowserContextKeyedServiceFactory` exists.
...
`Profile` creation and destruction. All constructors of PKSF registers and
unregisters themselves with the `BrowserContextDependencyManager` automatically.
