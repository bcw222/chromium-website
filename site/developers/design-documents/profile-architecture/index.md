...
The design has evolved a lot since then. In 2012, transitioned to the new model
described on this page. Rather than owning everything directly, `Profile` is now
a sort of handle object with minimal state. To prevent startup and teardown
issues, we use a `DependencyManager` and a two-phase shutdown model.

[TOC]

## Design Goals

*   **The `Profile` class must stay at a maintanable size.** It is tempting to
...
*   **Profile startup and shutdown must be robust.** When we started and only
    ...
    destruction. Now, we have over hundreds of services; we can not rely on
    manual ordering when we have so many components.
...
class FooService : public KeyedService {
 public:
  explicit FooService(Profile* profile) : profile_(profile) {
    ...
  }
...
`FooService`. Here's a typical example, where `FooService` depends on
`BarService` and `BazService`:

```cpp
class FooServiceFactory : public BrowserContextKeyedServiceFactory {
 public:
  static FooService\* GetForBrowserContext(BrowserContext\* context) {
    return static_cast<FooService\*>(
        GetInstance()->GetServiceForBrowserContext(browser_context, true));
  }

  static FooServiceFactory\* GetInstance() {
    return Singleton<FooServiceFactory>::get();
  }
 ...
 private:
  ...
  FooServiceFactory()
      : BrowserContextKeyedServiceFactory(
          "FooService",
          BrowserContextDependencyManager::Get()) {
    DependsOn(BarServiceFactory::Get());
    DependsOn(BazServiceFactory::Get());
  }

  virtual ~FooServiceFactory() = default;

  // BrowserContextKeyedServiceFactory:
  virtual BrowserContextKeyedService\* BuildServiceInstanceFor(
      content::BrowserContext\* context) const override {
    return new FooService(Profile::FromBrowserContext(context));
  }
};
```
...
BrowserContextKeyedServiceFactory provides an interface for to override
...
*   A constructor that associates the factory with the
            `ProfileDependencyManager` singleton, and makes `DependsOn()`
            declarations.
*   A `GetForBrowserContext()` method that wraps BCKSF, casting the result back
            to whatever type we need to return.
...
In addition, BCKSF provides these other knobs for
...
*   By default, BCKSF return NULL when given an Incognito profile.
    ...
    *   If you override `GetBrowserContextOwnInstanceInIncognito()` to create a
                new service for the incognito profile.
...
`FooService` accessed via `FooServiceFactory::GetForProfile(profile)`.
...
shutdown cannot accomodate these cases.

## Dependency Management Overview

With that in mind, let's look at how dependency management works. There is a
single `ProfileDependencyManager` singleton, which is what is alerted to
`Profile` creation and destruction. A BCKSF will register and unregister itself
with the ProfileDependencyManager. The job of the `ProfileDependencyManager` is
to make sure that individual services are created and destroyed in a safe
ordering.
...
BrowserContextKeyedServiceFactory::ProfileDestroyed()
